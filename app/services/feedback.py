...
class FeedbackLoopEngine:
    ...
    def update_system_learnings(
        ...
        positive_signals = {
            finding.signal_key for finding in pattern_report.winning_audiences + pattern_report.high_performing_creatives
        }
        negative_signals = {finding.signal_key for finding in pattern_report.budget_inefficiencies}
        ...
        for signal_key in tracked_signal_keys:
            ...
            if signal_key in positive_signals:
                adjustment = max(adjustment, 0.05 if comparison.performance_score >= baseline else adjustment)
            ...
            updated_signals.append(
                FeedbackSignal(
                    ...
                    successes=existing.successes + (1 if comparison.performance_score >= baseline else 0),
                    failures=existing.failures + (0 if comparison.performance_score >= baseline else 1),
                    last_updated=timestamp,
                )
            )
        ...
        snapshot = WeightSnapshot(
            ...
            signal_weights={key: signal.weight for key, signal in sorted(merged_weights.items())},
    ...
    def _collect_signal_keys(
        ...
        for audience in payload.audiences:
            ...
            signal_keys.add(f"audience:{str(audience_key).strip().lower().replace(' ', '_')}")
        for creative in payload.creatives:
            signal_keys.add(f"creative_type:{creative.type.strip().lower().replace(' ', '_')}")
